...
use crate::merkletreestorage::bitcaskdb::BitCaskDB;
...
type Tree<const N: usize, Value = Vec<u8>> = MerkleBIT<BitCaskTree<N, Value>, N>;
...
pub struct BitCaskTree<const N: usize = 32, Value: Encode + Decode = Vec<u8>> {
...
impl<const N: usize, Value: Encode + Decode> MerkleTree<N> for BitCaskTree<N, Value> {
    type Database = BitCaskDB<N>;
...
impl<const N: usize> BitCaskTree<N> {
    /// Creates a new `BitCaskTree`.  `depth` indicates the maximum depth of the tree.
    /// # Errors
    /// None.
    #[inline]
    pub fn new(path: &Path,depth: usize) -> BinaryMerkleTreeResult<Self> {
        let db = BitCaskDB::<N>::open(path)?;
    ...
    /// Creates a new `BitCaskTree`.  This method exists for conforming with the general API for the `MerkleBIT`
    ...
    pub fn decompose<'b>(self) -> (BitCaskDB<N>, usize) {
...
impl<const N: usize> Serialize for BitCaskTree<N> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {

        let mut state = serializer.serialize_struct("BitCaskTree", 3)?;
...
impl<'de,const N: usize> Deserialize<'de> for BitCaskTree<N> {
        ...
        D: Deserializer<'de>,
    {
        ...
        struct BitCaskTreeVisitor<const N: usize>;

        impl<'de,const N: usize> Visitor<'de> for BitCaskTreeVisitor<N> {
            type Value = BitCaskTree::<N>;

            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("struct BitCaskTree")
            }

            fn visit_seq<V>(self, mut seq: V) -> Result<BitCaskTree::<N>, V::Error>
            where
                V: SeqAccess<'de>,
            {
                ...
                let tree = BitCaskTree::<N>::new_with_root(path,root,depth);
                tree.map_err( |e| de::Error::custom(e.to_string()) )
            }

            fn visit_map<V>(self, mut map: V) -> Result<BitCaskTree::<N>, V::Error>
            where
                V: MapAccess<'de>,
            {
                let mut root = None;
        ...
        deserializer.deserialize_struct("BitCaskTree", FIELDS,  BitCaskTreeVisitor::<N>)
    }
}


impl<const N: usize> Default for BitCaskTree<N> {
    fn default() -> Self {
        BitCaskTree::<N> {
            tree: None,
            _value: PhantomData::default(),
...
impl<const N: usize> core::fmt::Debug for BitCaskTree<N>  {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("BitCaskTree")
         .field("path", &self.path)
         .finish()
    }
}

impl<const N: usize> Clone for BitCaskTree<N>   {
    fn clone(&self) -> Self {
        let db = BitCaskDB::<N>::open(&self.path).expect("Open database");
...
impl<const N: usize> minicbor::Encode for BitCaskTree<N>  {
...
impl<'b, const N: usize> minicbor::Decode<'b> for BitCaskTree<N> {
...
/* 
use sled::IVec;
pub struct BitCaskTreeIterator<'a,const N: usize = 32>{
    tree: Option<&'a Tree<N>>
}

impl<'a,const N: usize> BitCaskTreeIterator<'a,N> {
...
impl<'a,const N: usize> IntoIterator for BitCaskTreeIterator<'a,N> {
...
impl<const N: usize> Iterator for BitCaskTree<N> {
