...
pub struct Item {
    pub schema: Schema,
    pub rows: IndexMap<Key, ProofType>,
}
...
impl Store for MemoryStorage {
    ...
    async fn fetch_data(&self, table_name: &str, key: &Key) -> Result<Option<Row>> {
        let row = self
            .items
            .get(table_name)
            .and_then(|item| item.rows.get(key).map( |r| authcomp::unshallow::<Row>(r) ));

        row.transpose().map_err( | e | gluesql_core::result::Error::StorageMsg(e.to_string()))
    }

    async fn scan_data(&self, table_name: &str) -> Result<RowIter> {
        let rows: RowIter = match self.items.get(table_name) {
            Some(item) => Box::new(item.rows.clone().into_iter().map( |(k,r)| authcomp::unshallow::<Row>(&r).map( |x| (k,x)).map_err( | e | gluesql_core::result::Error::StorageMsg(e.to_string())) )),
            None => Box::new(empty()),
        };
...
impl MemoryStorage {
    ...
    pub fn append_data(&mut self, table_name: &str, rows: Vec<Row>) {
        println!("append_data {:?} {:?}",table_name,rows);
        if let Some(item) = self.items.get_mut(table_name) {
            for row in rows {
                self.id_counter += 1;
                println!("append_data key {:?}",self.id_counter);
                item.rows.insert(Key::I64(self.id_counter), authcomp::shallow(&row));
            }
        }
    }
    ...
    pub fn insert_data(&mut self, table_name: &str, rows: Vec<(Key, Row)>) {
        if let Some(item) = self.items.get_mut(table_name) {
            for (key, row) in rows {
                item.rows.insert(key, authcomp::shallow(&row));
            }
        }
    }
